...
def train_step(model: torch.nn.Module, dataloader: torch.utils.data.DataLoader,
               loss_fn: torch.nn.Module, optimizer: torch.optim):
  model.to(device)
  model.train()
  ...
  for batch , (x,y) in enumerate(dataloader):
    x,x = x.to(device=device, dtype=torch.float32), y.to(device=device, dtype=torch.int64)

    print(f"Input shape before model: {x.shape}")
...
def test_step(model: torch.nn.Module, dataloader: torch.utils.data.DataLoader,
  ...
  with torch.inference_mode():
    for batch, (x,y) in enumerate(dataloader):
      ...
      test_pred_lable= test_pred_logits.argmax(dim=1)
      test_acc += (test_pred_logits == y).sum().item()/ len(test_pred_lable)
...
def train(model: torch.nn.Module, train_dataloader: torch.utils.data.DataLoader,
  ...
  for epoch in tqdm(range(epochs)):
    ...
    print(
        f'epoch:{epoch+1}|'
        f'tr_loss :{train_loss:.5f}|'
        f'tr_acc: {train_acc:.5f}|'
        f'te_loss: {test_loss:.5f}|'
        f'te_acc: {test_acc:.5f}'
    )

    results['tr_loss'].append(train_loss)
    results['tr_acc'].append(train_acc)
    results['te_loss'].append(test_loss)
    results['te_acc:'].append(test_acc)
...
def eval_model(model: torch.nn.Module,data_loader : torch.utils.data.DataLoader,
  ...
  """returens a dic containing of model predicting resualt
  args:
  model (torch.nn.Module) : a pytorch model
  data_loader : the target dataset to pred
  loss_fn : te loss function of model
  accuracy_fn : An function to compare the model preds to the truth label

  returns:
      data : results of model making pred on data_loader"""
  ...
  #scale loss and acc to find the avrage
